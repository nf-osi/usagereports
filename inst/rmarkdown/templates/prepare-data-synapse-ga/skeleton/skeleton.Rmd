...
Optionally, use released study ids to create a reference table to include in the report.
Use whatever table and whatever attributes that makes sense.
...
We only visualize file metadata for files that were downloaded (by *non-Sage DCC* users) over the report period.
...
(Update `eval` for this other option.)
...
2. Manual curation of metadata based on filename/format and using rules which may be specified in your data model/ontology*.
...
2. Derive the proportions (though these may still be recalucated later on if categories are merged).
